...
export function CompliancePage({ secureScore, overview, dlpAlerts, purview, mcasAlerts, insiderRisk, attackSimulation, identity, devices, ca, securityIncidents, privilegedRoles, emailProtection }: {
  ...
  const allControls = useMemo(() => [
    ...
    { id:"PR.AA-02", cis:"CIS 5.1", iso:"A.8.2", gdpr:"Art. 32(4)", name:"Privileged Identity Management (PIM) & Role Governance", hasData:!!privilegedRoles?.configured, passed:(privilegedRoles?.totalPrivilegedUsers ?? 0) <= thresholds.maxAdmins, signal: privilegedRoles?.configured ? `${privilegedRoles.totalPrivilegedUsers ?? 0} privileged users (Max: ${thresholds.maxAdmins})` : "Privileged role data not collected", fix:"Enforce PIM just-in-time elevation and limit permanent global admins", link:"https://entra.microsoft.com/#view/Microsoft_AAD_IAM/PrivilegedIdentityManagementBlade" },
    ...
    { id:"PR.PT-02", cis:"CIS 9.2", iso:"A.8.7", gdpr:"Art. 32(1)(b)", name:"Safe Attachments & Safe Links Sandboxing", hasData:false, passed:false, signal: emailProtection?.configured ? `Cannot be verified from available data — Graph exposes MDO detections (${emailProtection.total ?? 0}) but not Safe Attachments/Safe Links policy state. Check the preset security policies directly.` : "MDO data not collected", fix:"Ensure Standard or Strict preset security policies are enabled in MDO", link:"https://security.microsoft.com/presetSecurityPolicies" },

    { id:"DE.AE-01", cis:"CIS 8.1", iso:"A.8.15", gdpr:"Art. 30", name:"Unified Audit Logging & Purview Signal Retention", hasData:false, passed:false, signal:purview?.configured ? `Cannot be verified from available data — Vigil365 reads sensitivity labels (${purview.labelCount ?? 0}), not unified audit-log configuration or retention. Confirm in the Purview audit portal.` : "Purview API disconnected", fix:"Connect Microsoft Purview API permission SecurityAlert.Read.All", link:"https://compliance.microsoft.com/auditlogsearch" },
