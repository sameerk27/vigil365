...
type NavPage = "overview" | "identity" | "devices" | "email" | "incidents" | "alertcenter" | "compliance" | "servicehealth" | "network" | "licenses" | "conditionalaccess" | "auditlog" | "signinmap" | "users" | "setup";
...
// ═══════════════════════════════════════════════════════════════════════════════
// SETUP WIZARD (Admin only) — enter Graph credentials in the browser, no JSON
// ═══════════════════════════════════════════════════════════════════════════════
function SetupPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      else if (d.testOk) { setResult({ ok: true, msg: "Saved and connected to Microsoft Graph successfully." }); showToast("Graph configured"); setClientSecret(""); await loadStatus(); }
      else { setResult({ ok: false, msg: `Saved, but the connection test failed: ${d.testError ?? "unknown error"}` }); await loadStatus(); }
